...
var _ = Describe("Workload cluster creation", func() {
	...
	Context("Creating workload cluster [REQUIRED]", func() {
		It("With default template to finish upgrade when Helm release is in pending-upgrade status", func() {
			...
			By("Overwriting the last Helm release status to pending-upgrade")
			hrp, err := getHelmReleaseProxy(ctx, bootstrapClusterProxy.GetClient(), clusterName, *hcp)
			Expect(err).NotTo(HaveOccurred())
			Expect(hrp).NotTo(BeNil())
			workloadClusterProxy := bootstrapClusterProxy.GetWorkloadCluster(ctx, namespace.Name, clusterName)
			Expect(workloadClusterProxy).NotTo(BeNil())
			SetHelmReleaseStatus(ctx, workloadClusterProxy, hrp.Spec.ReleaseNamespace, hrp.Spec.ReleaseName, helmRelease.StatusPendingUpgrade)
	...
	Context("Creating workload cluster [REQUIRED]", func() {
		It("With default template to finish upgrade when Helm release is in pending-rollback status", func() {
			...
			By("Overwriting the last Helm release status to pending-rollback")
			hrp, err := getHelmReleaseProxy(ctx, bootstrapClusterProxy.GetClient(), clusterName, *hcp)
			Expect(err).NotTo(HaveOccurred())
			Expect(hrp).NotTo(BeNil())
			workloadClusterProxy := bootstrapClusterProxy.GetWorkloadCluster(ctx, namespace.Name, clusterName)
			Expect(workloadClusterProxy).NotTo(BeNil())
			SetHelmReleaseStatus(ctx, workloadClusterProxy, hrp.Spec.ReleaseNamespace, hrp.Spec.ReleaseName, helmRelease.StatusPendingRollback)
